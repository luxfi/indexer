...
// testService creates a Service with a test database.
func testService(t *testing.T) *Service {
	t.Helper()
	tdb := testutil.NewTestDB(t)
	return testServiceWithDB(t, tdb)
}

// testServiceWithDB creates a Service with the given test database.
func testServiceWithDB(t *testing.T, tdb *testutil.DB) *Service {
	t.Helper()
	return &Service{
		config: Config{
...
// testServiceAndDB creates a Service with a test database and returns both.
func testServiceAndDB(t *testing.T) (*Service, *testutil.DB) {
	t.Helper()
	tdb := testutil.NewTestDB(t)
	return testServiceWithDB(t, tdb), tdb
}

func doGet(svc *Service, path string) *httptest.ResponseRecorder {
...
func TestIntegrationPipeline(t *testing.T) {
	...
	roDb, err := sql.Open("sqlite3", fmt.Sprintf("file:%s?mode=ro&_journal_mode=WAL", tdb.Path))
	if err != nil {
...
func queryTxResponse(t *testing.T, p *Service, hash []byte) map[string]any {
	t.Helper()
	rows, err := p.db.Query("SELECT * FROM transactions WHERE chain_id = ? AND hash = ? LIMIT 1",
