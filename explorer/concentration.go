...
// handleTokenDistribution serves GET /v1/explorer/tokens/{address_hash}/distribution.
func (s *Service) handleTokenDistribution(w http.ResponseWriter, r *http.Request) {
	ctx, cancel := context.WithTimeout(r.Context(), 10*time.Second)
	defer cancel()

	addr := chi.URLParam(r, "address_hash")
	dist, err := queryTokenDistribution(ctx, s.db, "address_current_token_balances", hexToBytes(addr))
	if err != nil {
		s.logger.Error("token distribution error", slog.String("error", err.Error()))
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to compute distribution"})
		return
	}
...
func (ss *StandaloneServer) tokenDistribution(r *http.Request) (any, int) {
	...
	dist, err := queryTokenDistribution(ctx, ss.db, ss.t.balances, addr)
	if err != nil {
