...
func (a *Adapter) InitSchema(db *sql.DB) error {
	schema := `
		...
		-- Corona quantum-resistant keys
		CREATE TABLE IF NOT EXISTS qchain_corona_keys (
			id TEXT PRIMARY KEY,
		...
		CREATE INDEX IF NOT EXISTS idx_qchain_keys_owner ON qchain_corona_keys(owner);
		CREATE INDEX IF NOT EXISTS idx_qchain_keys_type ON qchain_corona_keys(key_type);
		CREATE INDEX IF NOT EXISTS idx_qchain_keys_active ON qchain_corona_keys(revoked, valid_until);
...
func (a *Adapter) StoreCoronaKey(ctx context.Context, db *sql.DB, key *CoronaKey, vertexID string) error {
	_, err := db.ExecContext(ctx, `
		INSERT INTO qchain_corona_keys
...
func (a *Adapter) GetActiveKeys(ctx context.Context, db *sql.DB, owner string) ([]CoronaKey, error) {
	rows, err := db.QueryContext(ctx, `
		SELECT id, public_key, key_type, algorithm, security_level, owner, valid_from, valid_until, revoked, created_at
		FROM qchain_corona_keys
