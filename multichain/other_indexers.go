...
func (t *TonIndexer) ChainID() string    { return t.config.ID }
func (t *TonIndexer) ChainType() ChainType { return ChainTypeTon }
...
func (t *TonIndexer) IsRunning() bool { return atomic.LoadInt32(&t.running) == 1 }

func (t *TonIndexer) GetLatestBlock(ctx context.Context) (uint64, error) {
	url := fmt.Sprintf("%s/getMasterchainInfo", t.config.RPC)
	...
	seqno := result.Result.Last.Seqno
	t.mu.Lock()
	t.latestBlock = seqno
	t.mu.Unlock()
	return seqno, nil
...
func (t *TonIndexer) IndexBlock(ctx context.Context, seqno uint64) error {
	t.mu.Lock()
	t.indexedBlock = seqno
	t.stats.BlocksProcessed++
	t.stats.LastBlockTime = time.Now()
	t.mu.Unlock()
	return nil
