...
const (
	...
	DefaultRPCEndpoint = "http://localhost:9650/v1/chain/Q/rpc"
...
func (a *Adapter) InitSchema(ctx context.Context, store storage.Store) error {
	...
	_ = store.Exec(ctx, `CREATE INDEX IF NOT EXISTS idx_qchain_keys_owner ON qchain_corona_keys(owner)`)
	_ = store.Exec(ctx, `CREATE INDEX IF NOT EXISTS idx_qchain_keys_type ON qchain_corona_keys(key_type)`)
	_ = store.Exec(ctx, `CREATE INDEX IF NOT EXISTS idx_qchain_keys_active ON qchain_corona_keys(revoked, valid_until)`)
